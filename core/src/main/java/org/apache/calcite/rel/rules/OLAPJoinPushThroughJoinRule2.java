...
public class OLAPJoinPushThroughJoinRule2 extends RelRule<OLAPJoinPushThroughJoinRule2.Config> {
  ...
  public static final RelOptRule INSTANCE =
      new OLAPJoinPushThroughJoinRule2("OLAPJoinPushThroughJoinRule2",
  ...
  private void onMatchRight(RelOptRuleCall call) {
    ...
    Objects.requireNonNull(projectPermu, "projectPermu");
    ...
    final Mapping extendedProjectPerm =
        createAbstractTargetMapping(
            Mappings.append(projectPermu,
    ...
    final Mappings.TargetMapping tempMapping =
        Mappings.createShiftMapping(aCount + bCount + cCount,
            0, 0, aCount,
            aCount + cCount, aCount, bCount, aCount, aCount + bCount, cCount);
    final Mappings.TargetMapping thruProjectMapping =
        Mappings.multiply(extendedProjectPerm, createAbstractTargetMapping(tempMapping));
    ...
    final Join newBottomJoin =
        bottomJoin.copy(bottomJoin.getTraitSet(),
            newBottomCondition,
            relA,
            relC,
    ...
    @SuppressWarnings("SuspiciousNameCombination") final Join newTopJoin =
        topJoin.copy(topJoin.getTraitSet(),
            newTopCondition,
            newBottomJoin,
            relB,
            topJoin.getJoinType(),
      ...
      return withOperandSupplier(b0 ->
          b0.operand(joinClass).inputs(b1 ->
                  b1.operand(Project.class)
                      .predicate(input -> input.getPermutation() != null)
                      .inputs(
                          b2 -> b2.operand(joinClass).inputs(
                              b11 -> b11.operand(RelNode.class).anyInputs(),
                              b12 -> b12.operand(RelNode.class)
                                  .predicate(relNode -> !(relNode instanceof TableScan))
                                  .anyInputs())),
